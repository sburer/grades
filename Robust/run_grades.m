...
% Loop over 50 instances in tot_G_nom

warning('Only solving first problem')

for iter = 1:1 % length(tot_G_nom)
  ...
  dat.w     = 0.2*ones(5,1); % This weight is hard-coded
  ...
  if dat.norm == 0
    dat.f_norm_rhs = ceil(0.1*dat.m*dat.n);
  else
    dat.f_norm_rhs = 0.1;
  ...
  save mat/output3_norm0_eps0.5_unc0.1.mat output_us feedback_us data_us
